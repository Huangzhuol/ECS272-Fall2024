...
export default {
  data() {
    return {
      ...
      nodes: [] as any[], // Sankey nodes
      links: [] as any[], // Sankey links
    };
  },
  props: {
    highlightedNode: {
  ...
  methods: {
    onResize() {
      ...
      if (target) {
        ...
        d3.select('#sankey-svg').selectAll('*').remove(); // Clear previous elements
        this.initSankeyChart(); // Reinitialize with updated size
      }
    },
    ...
    prepareSankeyData() {
      ...
      dimensions.forEach((dim, idx) => {
        if (idx === dimensions.length - 1) return; // Skip last dimension
    ...
    initSankeyChart() {
      ...
      node
        ...
        .on("mouseout", function () {
          d3.select(this).style("fill", "#2d7dd2"); // Revert node color

          // Revert related links' color
          linkSelection
            .style("stroke", "#888");
        });
  ...
  watch: {
    ...
    highlightedNode(data) {
      d3.selectAll('.node rect')
        .transition()
        .duration(300) // Duration for the reset animation
        .style('fill', '#2d7dd2');
      ...
      if (data) {
        // Animate the highlighting of relevant nodes
        d3.selectAll('.node rect')
          .filter((nodeData: any) => nodeData.name.includes(`${data.type}_${data.level}`))
          .transition()
          .duration(500) // Duration for the highlight animation
          .style('fill', '#ff5733'); // Highlight color

        // Animate the highlighting of relevant links
        d3.selectAll('.link')
          .filter((linkData: any) =>
          ...
          .transition()
          .duration(500) // Duration for the highlight animation
          .style('stroke', '#ff5733') // Highlight color for links
          .style('stroke-opacity', 0.8); // Make highlighted links more visible
      }
    }
  },
  mounted() {
